...
def dfct_eval(x, fxn_list, n, ns, p, q, dfct_idx):
    ...
    # Asssumes a defect is tracled for every state.
    # Singular jacobian results if this is not the case.
    yk = yk_full[dfct_idx]
    fk = fxn(yk_full)   
    ykp1 = ykp1_full[dfct_idx]
    fkp1 = fxn(ykp1_full)
    ykc = center_state_vector_constructor(yk_full, ykp1_full,
                                          p, q, hk, fxn_list)
    fc = fxn(ykc)
...
def state_parser(x, m, p):
    n_states = len(x) / m
    nlp_var_list = []
    for i in range(n_states):
        nlp_var_list.append(x[i*m:(i+1)*m])
    state_list = nlp_var_list[0:p]
    control_list = nlp_var_list[p:]
